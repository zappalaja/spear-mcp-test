"""MCP server with dynamic SPEAR web portal navigation - NC File Reading Soon!!"""
...
async def get_spear_file_info(file_path: str) -> Dict:  #NEEDS WORK#################
    """
    Get detailed information about a specific SPEAR NetCDF file.
    
    Args:
        file_path: Full path to the NetCDF file (e.g., "historical/r1i1p1f1/Amon/tas/gr3/v20210201/tas_Amon_GFDL-SPEAR-MED_historical_r1i1p1f1_gr3_192101-201412.nc")
        
    Returns:
        Dict containing file metadata, dimensions, variables, and access URL
    """
    clean_path = _clean_path(file_path)
    full_file_path = f"{SPEAR_BASE_PATH}/{clean_path}"
    file_url = f"{SPEAR_BASE_URL}/{full_file_path}"
    
    # Validate the URL
    validate_spear_url(file_url)
    
    try:
        # Try to open the dataset to get metadata
        dataset = await open_dataset(file_url)
        ...
        return {
            ...
            "time_range": _extract_time_range(dataset) if 'time' in dataset.dims else None
        }
        
    except Exception as e:
        logger.error(f"Error accessing file '{file_path}': {e}")
        # Return basic info even if we can't open the dataset
        return {
            "file_path": clean_path,
            "file_url": file_url,
            "error": str(e),
...
async def search_spear_variables(scenario: str, variable_pattern: str = "", 
    ...
    try:
        # Browse the scenario directory
        scenario_content = await browse_spear_directory(scenario)
        
        # Look through available runs
        for run in scenario_content.get("directories", []):
            ...
            # Look through frequency directories if specified
...
def _get_known_structure(path: str) -> SPEARNavigationResult:
    ...
    # Root level - show scenarios
    if not path:
        return SPEARNavigationResult("", SPEAR_SCENARIOS, [], None)
    
    # Scenario level - show runs
    ...
    # Run level - show frequency directories
    ...
    # Frequency level - show common variables
    if len(path_parts) == 3:
        freq = path_parts[2]
        if freq == "Amon":
